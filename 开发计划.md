...
| 包管理 | 后端用 **uv**（符合个人习惯） |
| 模型平台 | **全阿里云百炼**（云端 API，不本地部署模型） |
| 模型配置 | qwen3.7-plus / deepseek-v4-pro-0813 / deepseek-v4-flash-0731 / text-embedding-v3 |
...
flowchart TD
    ...
    subgraph Bailian[阿里云百炼 云端API]
        M1[qwen3.7-plus<br/>分诊/结构化/报告]
        M2[deepseek-v4-pro-0813<br/>开放对话/共情]
...
| 用途 | 模型 | 角色定位 | 关键能力 |
|---|---|---|---|
| 分诊/结构化提取/报告主力 | `qwen3.7-plus` | 量表结果结构化、风险分类、报告生成 | 1M 上下文、结构化输出、function calling |
| 开放对话/共情干预 | `deepseek-v4-pro-0813` | 多轮共情对话、心理语义分析 | 百万上下文、中文心理场景表现好 |
| 高频/兜底/报告辅助 | `deepseek-v4-flash-0731` | 轻量任务、对话摘要、成本敏感场景 | 低延迟、低成本 |
| RAG 向量化 | `text-embedding-v3` | 心理知识库切片向量化 | 中文 embedding |

**Provider 抽象层**：统一百炼 OpenAI 兼容协议，`.env` 配置 `DASHSCOPE_API_KEY` + 4 个模型名，便于后续切换/扩容其他平台。
...
│   ├── app/
│   │   ├── main.py               # FastAPI 入口
│   │   ├── api/                  # 路由: auth/assessment/chat/report
│   │   ├── agents/               # intake/assessment/intervention/crisis
│   │   ├── graph/                # LangGraph state/router/pipeline
│   │   ├── scales/               # 量表库: phq_a/scared 计分规则(自研核心)
│   │   ├── rag/                  # 知识库构建+检索
│   │   ├── reports/              # 报告模板+生成器(WeasyPrint)
│   │   ├── safety/               # 危机关键词词典+升级
│   │   ├── models/               # 数据模型 SQLAlchemy/Pydantic
│   │   ├── core/                 # 配置/LLM Provider 抽象层(百炼)
│   │   └── utils/
│   ├── tests/                    # 量表计分单测(核心)
│   ├── pyproject.toml            # uv 管理
│   └── Dockerfile                # Python + 系统库(cairo/pango) + WeasyPrint
├── data/
│   ├── scales/                   # 量表题库数据
│   ├── knowledge/                # RAG 心理学知识源文档
│   ├── chroma/                   # 向量库持久化(挂载卷)
│   └── psycheflow.db             # SQLite 数据库
├── docker-compose.yml            # chroma + backend + frontend
├── .env.example                  # 百炼 Key + 模型配置模板
└── 开发计划.md
```
